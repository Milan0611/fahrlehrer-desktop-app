...
* **Echtzeit-Interaktion:** Die Vorbereitung für anonyme Umfragen und kompetitive Quiz-Elemente (analog zu Kahoot) ist durch das zustandsbasierte Template-System im Frontend bereits angelegt und erfordert im nächsten Schritt lediglich die Anbindung eines WebSocket-Backends.
...
Um die vollständige Dynamik der Unterrichtsinhalte bei identischem Client zu realisieren, nutzt das Projekt ein strikt datengetriebenes Konzept nach dem Prinzip **"Content as Data"**. Die Architektur ist so konzipiert, dass das Gesamtsystem über **zwei zentrale Stellschrauben** skaliert und gewartet werden kann:

1. **Neue Folieninhalte hinzufügen:** Hierfür muss kein einziger Buchstabe Code angefasst werden. Man erweitert lediglich die entsprechende JSON-Datei im `lessons`-Ordner um ein neues Folien-Objekt.
2. **Neue Features / Folientypen integrieren:** Soll ein völlig neuer Folientyp entstehen (z. B. eine interaktive Umfrage), erstellt man einfach ein neues View-Template (z. B. `PollTemplate.tsx`) und registriert es im zentralen `SlideRenderer`. Bestehende Komponenten bleiben davon komplett unberührt.
