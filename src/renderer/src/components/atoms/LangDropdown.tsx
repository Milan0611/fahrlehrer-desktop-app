import { AvailableLanguage, availableLanguages } from '../../lessons/LessonLanguages';

/**
 * Schnittstellendefinition für das Sprachauswahl-Atom.
 * Definiert den Lesestatus (currentLang) und den Schreibzugriff (onChange) auf den
 * übergeordneten Sprach-State.
 */
interface LangDropdownProps {
  currentLang: AvailableLanguage;
  onChange: (newLang: AvailableLanguage) => void; 
}

/**
 * UI-Komponente zur Auswahl der Applikationssprache.
 * Agiert als kontrollierte Komponente (Controlled Component). Sie besitzt keinen eigenen State,
 * sondern erhält den aktuellen Wert vom Parent und meldet Änderungen über den Event-Handler
 * (Lifting State Up) an diesen zurück.
 */
export const LangDropdown = ({ currentLang, onChange }: LangDropdownProps) => {
  return (
    <div className="relative inline-block">
      <select
        value={currentLang}
        onChange={(e) => onChange(e.target.value as AvailableLanguage)} 
        className="appearance-none bg-gray-800 text-white font-bold py-2 pl-4 pr-10 rounded-lg shadow-lg border-2 border-next-level-green cursor-pointer focus:outline-none"
      >
        {/* Generiert die Auswahloptionen dynamisch aus der zentralen Sprach-Registry */}
        {availableLanguages.map((langKey) => (
          <option key={langKey} value={langKey}>
            {langKey.toUpperCase()}
          </option>
        ))}
      </select>
      
      <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-next-level-green">
        <svg className="fill-current h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
          <path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z" />
        </svg>
      </div>
    </div>
  );
};